...
```
Input(29)
  └── Dense(256) → BatchNorm → Swish → Dropout(0.30)
        └── ResBlock(128): skip+[Dense→BN→Swish→Drop(0.20)→Dense]+Add→BN→Swish
              └── ResBlock(64):  skip+[Dense→BN→Swish→Drop(0.10)→Dense]+Add→BN→Swish
